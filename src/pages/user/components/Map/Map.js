...
const Map = ({ hotels, coords, setChildClicked, radius }) => {
    ...
    return (
        <div className={classes.mapContainer}>
            ...
            >
                {hotels.length && hotels.map((hotel, i) => (
                    <div
                        key={i}
                        ...
                        {!matches
                            ...
                            : (
                                <Paper elevation={3} className={classes.paper}>
                                    <Typography className={classes.typography} variant="subtitle2" gutterBottom> {hotel.name} </Typography>
                                    <img
                                        className={classes.pointer}
                                        src='https://suretyhotel.com/wp-content/uploads/2021/01/Deluxe-King-and-Surety-King-scaled-1280x0-c-default.webp'
                                    />
                                    <Rating name="read-only" size="small" value={Number(hotel.rating)} readOnly />
                                </Paper>
                            )}
                    </div>
                ))}
